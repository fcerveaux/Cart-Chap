...
const exportLoaderBtn = document.getElementById("exportLoaderBtn");
...
const leaveWarn = document.getElementById("leaveWarn");
...
function render() {
  titleEl.value = game.title || "";
  subjectEl.value = game.subject || "";
  durationEl.value = game.duration || "";
...
function bind() {
  titleEl.oninput = () => {
    game.title = titleEl.value;
    markDirty();
  };
  subjectEl.oninput = () => {
    game.subject = subjectEl.value;
    markDirty();
  };
  durationEl.oninput = () => {
    game.duration = parseInt(durationEl.value || "0", 10) || null;
    markDirty();
  };
  ...
  cardsEl.addEventListener("click", (e) => {
    ...
    if (act === "del") {
      ...
      [game.cards[i - 1], game.cards[i]] = [game.cards[i], game.cards[i - 1]];
    } else if (act === "down" && i < game.cards.length - 1) {
      [game.cards[i + 1], game.cards[i]] = [game.cards[i], game.cards[i + 1]];
    }
    markDirty();
    render();
  ...
  shareHashBtn.onclick = async () => {
    const url = await toHashURL(game);
